...
{/* SYNC_API_DOCS: Rate limit table must stay in sync with:
     - tuco-app/src/lib/apiRateLimiter.ts RATE_LIMIT_PRESETS (enforcement)
     - tuco-app/src/lib/availabilityThrottle.ts (daily caps, device gaps)
     - tuco-app/src/lib/messageSender.ts (daily line limits)
     - tuco-app/src/app/integrations/page.tsx (API Reference table in UI)
     - tuco-app/src/components/ApiInstructions.tsx (shared API docs component)
