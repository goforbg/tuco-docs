...
|---|---|
| **Check iMessage availability** (`/api/check-availability`, `/api/leads/check-availability`) | 70 checks/day per line; high-volume checks may experience slight delays |
| **Send message** (`/api/messages`, `/api/leads/send`) | 60 req/min |
...
<Note>
**Availability checks:** Each line in your workspace can perform up to 70 availability checks per day.
If you have multiple lines, your total daily capacity is higher. When all lines are exhausted for the day,
you receive a `429` response. High-volume checks may experience slight delays to ensure accuracy.
</Note>
