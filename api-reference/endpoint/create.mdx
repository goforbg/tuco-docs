...
**All duplicates** – status `409 Conflict`. No leads are created; the body includes `code: "DUPLICATE_LEADS"`, `duplicateCount`, and `duplicates` (each with `input` and `existingLead` including `_id`) so you can reconcile with existing records.
