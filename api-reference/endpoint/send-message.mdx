...
<ParamField body="message" type="string" required>
  The text content to send. Supports plain text. Personalization placeholders
  like `{{firstName}}` are resolved from the lead at send time.
</ParamField>
...
```bash Minimal (defaults to iMessage)
...
```bash With alt contacts + time window
curl -X POST "https://app.tuco.ai/api/messages" \
  -H "Authorization: Bearer tuco_sk_xxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    ...
    "recipientPhone": "+12025551234",
    "altPhone1": "+12025559876",
    "altEmail1": "prospect@icloud.com",
...
```bash Scheduled + fallback SMS on failure
curl -X POST "https://app.tuco.ai/api/messages" \
  -H "Authorization: Bearer tuco_sk_xxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
...
resp = requests.post(
    "https://app.tuco.ai/api/messages",
...
const resp = await fetch("https://app.tuco.ai/api/messages", {
  method: "POST",
  headers: {
    Authorization: "Bearer tuco_sk_xxxxxxxxxxxxx",
...
- **Device gap** is met (min time between sends from same device)
- **Contact gap** is met (min time between first messages to different contacts)
...
| Status | When | Example |
|--------|------|---------|
| `400` | Missing `message` | `{ "error": "Missing required field: message" }` |
